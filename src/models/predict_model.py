...
if __name__ == "__main__":
    ...
    model, modelNumber, attentionModel = loadModel()
    #DataLoader
    dataObj = DataLoader()
    ...
    results = []
    csvSavePath = f'{PREDICTIONS_RESULTS_SAVE_PATH}/{modelNumber}/predictions.csv'
    ...
    if MODEL_TYPE == "cnn_attention":
        attentionMaps = attentionModel.predict(X_test)

        # organize the maps by genre
        uniqueLabs = np.unique(np.argmax(y_test,axis=1))
        genreAttentionDict = {lab:[] for lab in uniqueLabs}
        for attn,label in zip(attentionMaps,y_test):
            label = np.argmax(label)
            genreAttentionDict[label].append(attn)

        # caluclate the mean
        genreAvgDict = {}
        for genre, maps in genreAttentionDict.items():
            genreAvgDict[genre] = np.mean(maps, axis=0)  # shape: (H, W)
        vizObj = Visualization()
        vizObj.getAttentionLayer(genreAvgDict)
